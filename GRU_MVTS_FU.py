...
class Classifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, test_X, test_y, plot_graph=False):
        X = pad_sequences(X)
        ...
        history = model.fit(X, y, batch_size=self.n_batch_size, nb_epoch=self.n_epochs, show_accuracy=True,
...
def fit_one_timeseries(dataset, ts_index, future=[(201810, 201814), (201814, 201819)], neurons=4, plot_graph=False):
    ...
    for from_index, to_index in future:
        ...
        # make a prediction
        yhat = clf.model_.predict(test_X)
        yhat_proba = clf.model_.predict_proba(test_X)
